  ...
  if (!isAdmin) {
    return React.createElement('div', { className: 'documents-container' },
      React.createElement('div', { className: 'access-denied glass-effect' },
        ...
        React.createElement('h1', null, 'Доступ ограничен'),
        React.createElement('p', null, 'У вас нет прав для доступа к этому разделу'),
        React.createElement('p', { className: 'access-subtext' }, 
          'Только администраторы могут управлять документами для обучения AI-ассистента'
        )
      )
    );
  }
  ...
  return React.createElement('div', { className: 'documents-container' },
    React.createElement('div', { className: 'documents-header' },
      React.createElement('div', { className: 'header-content' },
        ...
        React.createElement('div', { className: 'header-text' },
          ...
          //   'Загружайте документы для обучения AI-ассистента. Система автоматически проанализирует содержимое и расширит базу знаний.'
          // )
        )
      )
    ),
    ...
    React.createElement('div', { className: 'upload-section' },
      React.createElement('div', { className: 'upload-card' },
        ...
        React.createElement('h3', null, 'Загрузить новый документ'),
        React.createElement('p', { className: 'upload-description' },
          'Загрузите файл для расширения знаний AI-ассистента'
        ),
        ...
        React.createElement('label', {
          ...
          uploading 
            ? React.createElement('div', { className: 'upload-loading' },
                React.createElement('div', { className: 'spinner' }),
                'Загрузка...'
              )
            : React.createElement('div', { className: 'upload-btn-content' },
                React.createElement('div', { className: 'upload-btn-icon' }),
                'Выбрать файл'
              )
        ),
        React.createElement('input', {
      ...
      error && React.createElement('div', { className: 'message error-message' }, 
        error
      ),
      
      success && React.createElement('div', { className: 'message success-message' }, 
        success
      )
    ),
    ...
    React.createElement('div', { className: 'documents-list' },
      React.createElement('div', { className: 'list-header' },
        React.createElement('h2', null, 'Загруженные документы'),
        React.createElement('span', { className: 'documents-count' }, documents.length)
      ),
      
      documents.length === 0 
        ? React.createElement('div', { className: 'empty-state' },
            React.createElement('div', { className: 'empty-icon' }),
            React.createElement('h3', null, 'База знаний пуста'),
            React.createElement('p', null, 'Загрузите первый документ, чтобы начать обучение AI-ассистента')
          )
        : React.createElement('div', { className: 'documents-grid' },
            documents.map(function(doc, index) {
              return React.createElement('div', { 
                ...
                React.createElement('div', { className: 'document-header' },
                  React.createElement('div', { 
                    className: 'document-icon file-type-' + getFileType(doc.originalName)
                  }),
                  React.createElement('div', { className: 'document-info' },
                    React.createElement('h3', { className: 'document-name' }, doc.originalName),
                    React.createElement('span', { className: 'document-badge' }, 
                      getFileType(doc.originalName).toUpperCase()
                    )
                  ),
                  React.createElement('button', {
                    className: 'delete-btn',
                    onClick: function() { handleDeleteDocument(doc._id); },
                    title: 'Удалить документ'
                  })
                ),
                React.createElement('div', { className: 'document-meta' },
                  React.createElement('span', { className: 'meta-item' },
                    React.createElement('div', { className: 'meta-icon calendar-icon' }),
                    formatDate(doc.createdAt)
                  ),
                  doc.uploadedBy && React.createElement('span', { className: 'meta-item' },
                    React.createElement('div', { className: 'meta-icon user-icon' }),
                    doc.uploadedBy.fullName
                  )
                ),
                React.createElement('div', { className: 'document-status' },
